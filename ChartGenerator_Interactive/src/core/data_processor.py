...
class DataProcessor:
    ...
    def _detect_delimiter(self, filepath: str) -> str:
        """Detect the delimiter of a CSV file using csv.Sniffer, fallback to comma if not a common delimiter."""
        expected_delimiters = {',', ';', '\t', '|'}
        try:
            with open(filepath, 'r', encoding='utf-8', errors='replace') as f:
                ...
                if detected not in expected_delimiters:
                    logger.warning(f"Detected unusual delimiter '{detected}', falling back to comma.")
                    return ','
                return detected
        except Exception as e:
