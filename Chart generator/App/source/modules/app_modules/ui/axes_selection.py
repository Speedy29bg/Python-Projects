...
class AxesSelectionFrame:
    ...
    def clear_selections(self):
        """Clear all axis selections and listbox contents"""
        self.clear_listboxes()
        logging.info("Cleared all axes selections")
    ...
    def get_x_column(self):
        """Return the first selected X-axis column, or None if nothing is selected."""
        x_indices = self.x_listbox.curselection()
        if not x_indices:
            return None
        return self.x_listbox.get(x_indices[0])
        
    def get_primary_y_columns(self):
        """Get the currently selected primary Y-axis columns
        
        Returns:
            list: Selected column names for primary Y-axis
        """
        # Get indices
        y_indices = self.y_listbox.curselection()
    ...
    def get_secondary_y_columns(self):
        """Get the currently selected secondary Y-axis columns
        
        Returns:
            list: Selected column names for secondary Y-axis
        """
        # Get indices
        secondary_indices = self.secondary_listbox.curselection()
